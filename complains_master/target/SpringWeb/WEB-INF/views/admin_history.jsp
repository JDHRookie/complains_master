...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-md-2 bootstrap-admin-col-left">
            <ul class="nav navbar-collapse collapse bootstrap-admin-navbar-side">
                ...
                <li>
                    <a href="admin_complaininfo"><i class="glyphicon glyphicon-chevron-right"></i> 工单状态信息</a>
                </li>
                <li>
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default bootstrap-admin-no-table-panel">
                        <div class="panel-heading">
                            <div class="text-muted bootstrap-admin-box-title">工单状态信息</div>
                        </div>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <table id="data_list" class="table table-hover table-bordered" cellspacing="0" width="100%">
                        ...
                        <!---在此插入信息-->
                        <c:forEach var="historyData" items="${historyList}" >

                            <tbody>
                            <td>${historyData.number}</td>
                            <td>${historyData.income}</td>
                            <td>${historyData.contact}</td>
                            <td>${historyData.tel}</td>
                            <td>
                                <c:if test="${historyData.status ==0}">未处理</c:if>
                                <c:if test="${historyData.status ==1}">处理中</c:if>
                                <c:if test="${historyData.status ==2}">已完成</c:if>
                            </td>
                            <td>${historyData.type}</td>
                            <td>${historyData.content}</td>
                            <tb><button type="button" class="btn btn-danger btn-xs" onclick="deleteHistory(${historyData.hid})">删除</button></tb>
                            </tbody>
                        </c:forEach>
                    </table>
                </div>
    ...
    <script type="text/javascript">
        function deleteHistory(hid) {
            con=confirm("是否删除这条记录?");
            if(con==true){
                location.href = "${pageContext.request.contextPath}/deleteHistoryData?hid="+hid;
            }
            <c:if test="${sessionScope.flag17!=0}">
            alert("删除成功!")
            </c:if>
        }
    </script>
